...
if PERINTAH == "login" then
    ...
    if cookie and cookie:find("_|WARNING") then
        ...
        local kandidat = {}
        if pkg then kandidat[#kandidat+1] = pkg end
        for _, p in ipairs(cfg.pkgs or {}) do
            if p ~= pkg then kandidat[#kandidat+1] = p end
        end
        local ketemu = nil
        for _, p in ipairs(kandidat) do
            local ck = cookie_dari_client(p)
            if ck then
                -- uname TERKUBUR di base64 (tengah cookie, antara |_ dan .),
                -- harus di-decode dulu -- pola teks biasa GAK KENA.
        ...
        if not ketemu then
            ...
            local adaTapiBeda = nil
            for _, p in ipairs(kandidat) do
                local ck = cookie_dari_client(p)
                if ck then adaTapiBeda = uname_dari_cookie(ck) or "?"; break end
            end
            if adaTapiBeda then
                err(("Client login '%s', bukan '%s'."):format(adaTapiBeda, akun))
...
elseif io.open(PID_FILE, "r") then
    bersih(cfg, "selesai")
end--        * wake-lock CPU pas start (worker + client gak ditidurin layar idle).
--        * AUTO-REJOIN pinter: bridge diem TAPI client masih di game (pkg_running) ->
--          cukup DIBANGUNIN (bawa ke depan), JANGAN kill+buka. kill cuma kalau
--          beneran keluar dari layar game.
--
