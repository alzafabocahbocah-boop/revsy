...
# v1.1: pakai $HOME, BUKAN /tmp -- Termux gak punya /tmp, dan akibatnya
# bagian PALING PENTING dari skrip ini (daftar berkas) gagal diam-diam.
# Yang keliatan cuma "Directory nonexistent" nyempil di tengah keluaran.
TA="${TMPDIR:-$HOME}/_banding_a.txt"
TB="${TMPDIR:-$HOME}/_banding_b.txt"
su -c "find /data/data/$PA -type f -not -path '*/lib/*' -not -path '*/cache/*' 2>/dev/null" \
    | sed "s|/data/data/$PA/||" | sort > "$TA"
su -c "find /data/data/$PB -type f -not -path '*/lib/*' -not -path '*/cache/*' 2>/dev/null" \
    | sed "s|/data/data/$PB/||" | sort > "$TB"
comm -23 "$TA" "$TB" | head -40 | sed 's/^/    /'
...
comm -12 "$TA" "$TB" | while read -r f; do
...
# v1.1: lib/ DIKECUALIIN. Kata "ROBLOSECURITY" itu ketanam di kode Roblox
# sendiri (nama cookie-nya), jadi libroblox.so KENA di client yang belum
# pernah login sekalipun. Positif palsu yang bikin salah simpul.
su -c "grep -rla ROBLOSECURITY /data/data/$PA/ --exclude-dir=lib 2>/dev/null" | while read -r f; do
...
HASIL=$(su -c "grep -rla ROBLOSECURITY /data/data/$PB/ --exclude-dir=lib 2>/dev/null")
if [ -z "$HASIL" ]; then
